...
void TMVAClassificationApplication( TString myMethodList = "" )
{
 ...
 //  TTree* theTree = (TTree*)input->Get("TreeSelectron");
   TTree* theTree = (TTree*)input->Get("TreeBKion");
   ...
   for (Long64_t ievt=0; ievt<theTree->GetEntries();ievt++) {
    ...
    //  if (Use["CutsGA"]) {
      ...
      if (Use["Likelihood"   ])   histLk     ->Fill( reader->EvaluateMVA( "Likelihood method"    ) );//chosed
      ...
      if (Use["DNN_CPU"]) histDnnCpu->Fill(reader->EvaluateMVA("DNN_CPU method"));//chosed
      if (Use["BDT"          ])   histBdt    ->Fill( reader->EvaluateMVA( "BDT method"           ) );//chosed
   ...
   TFile *target  = new TFile( "TMVApp_BKion.root","RECREATE" );
