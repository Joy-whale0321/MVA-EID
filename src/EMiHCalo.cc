...
EMiHCalo::EMiHCalo(const std::string &name, const std::string &file):
    SubsysReco(name),
    _outfilename(file),
    _outfile(nullptr),
    _tree(nullptr)
{
    std::cout << "EMiHCalo::EMiHCalo(const std::string &name) Calling ctor" << std::endl;
}

//____________________________________________________________________________..
EMiHCalo::~EMiHCalo()
{
    std::cout << "EMiHCalo::~EMiHCalo() Calling dtor" << std::endl;
}

//____________________________________________________________________________..
int EMiHCalo::Init(PHCompositeNode *topNode)
{
    std::cout << topNode << std::endl;
    std::cout << "EMiHCalo::Init(PHCompositeNode *topNode) Initializing" << std::endl;
...
//____________________________________________________________________________..
int EMiHCalo::process_event(PHCompositeNode *topNode)
{
    ...
    if (!vertexmap)
    {
        std::cout << "EMiHCalo::process_event - Fatal Error - GlobalVertexMap node is missing. Please turn on the do_global flag in the main  macro in order to reconstruct the global vertex." << std::endl;
    ...
    if (vertexmap->empty())
    {
        std::cout << "EMiHCalo::process_event - Fatal Error - GlobalVertexMap node is empty. Please turn on the do_global flag in the main macro  in order to reconstruct the global vertex." << std::endl;
    ...
    if (!towerGeom)
    {
        std::cout << "EMiHCalo::process_event: Cannot find CEMC RawTowerGeomContainer!" << std::endl;
    }
    ...
    if (!clustersEM)
    {
        std::cout << "EMiHCalo::process_event: cannot find cluster container " << "CLUSTERINFO_CEMC" << std::endl;
    }

    // clustersIH = findNode::getClass<RawClusterContainer>(topNode, "CLUSTERINFO_HCALIN");
    // if (!clustersIH)
    // {
    //     std::cout << "EMiHCalo::process_event: cannot find cluster container " << "CLUSTERINFO_HCALIN" << std::endl;
    // }

    // clustersOH = findNode::getClass<RawClusterContainer>(topNode, "CLUSTERINFO_HCALOUT");
    // if (!clustersOH)
    // {
    //     std::cout << "EMiHCalo::process_event: cannot find cluster container " << "CLUSTERINFO_HCALOUT" << std::endl;
    // }
    ...
    if(!EMCAL_Container)
    {
        std::cout << "EMiHCalo::process_event: TOWERINFO_CALIB_CEMC not found!!!" << std::endl;
    }

    IHCAL_Container = findNode::getClass<TowerInfoContainer>(topNode, "TOWERINFO_CALIB_HCALIN");
    if(!IHCAL_Container)
    {
        std::cout << "EMiHCalo::process_event: TOWERINFO_CALIB_HCALIN not found! Aborting!" << std::endl;
    ...
    if(!EMCalGeo)
    {
        std::cout << "EMiHCalo::process_event: TOWERGEOM_CEMC not found!!!" << std::endl;
    }

    IHCalGeo = findNode::getClass<RawTowerGeomContainer>(topNode, "TOWERGEOM_HCALIN");
    if(!IHCalGeo)
    {
        std::cout << "EMiHCalo::process_event: TOWERGEOM_HCALIN not found!!!" << std::endl;
    }

    OHCalGeo = findNode::getClass<RawTowerGeomContainer>(topNode, "TOWERGEOM_HCALOUT");
    if(!OHCalGeo)
    {
        std::cout << "EMiHCalo::process_event: TOWERGEOM_HCALOUT not found!!!" << std::endl;
    }
    ...
    // {
    //     std::cout << "EMiHCalo::process_event: TOWER_CALIB_CEMC not found!!!" << std::endl;
    // }
...
//____________________________________________________________________________..
int EMiHCalo::End(PHCompositeNode *topNode)
{
    std::cout << topNode << std::endl;
    _outfile->cd();
    _outfile->Write();
...
void EMiHCalo::ResetTreeVectors()
{
    _run_test.clear();
    // EMCal vectors
    _emcalgeo_id.clear();
...
void EMiHCalo::FillTree()
{
    ...
    TowerInfo *tInfo_emc = nullptr;
    TowerInfo *tInfo_ihc = nullptr;
    TowerInfo *tInfo_ohc = nullptr;

    for(unsigned int iem = 0; iem < EMCAL_Container->size(); iem++)
    {
        tInfo_emc = EMCAL_Container->get_tower_at_channel(iem); 
        ...
        if(!tInfo_emc->get_isGood())
        {
            continue;
        }

        //if(tInfo_emc->get_energy() < 0.2) continue;
        ...
        _emcal_e.push_back(tInfo_emc->get_energy());
        ...
        _emcal_time.push_back(tInfo_emc->get_time());
        _emcal_chi2.push_back(tInfo_emc->get_chi2());
        _emcal_pedestal.push_back(tInfo_emc->get_pedestal());
    }

    for(unsigned int ihcal = 0; ihcal < IHCAL_Container->size(); ihcal++)
    {
        tInfo_ihc = IHCAL_Container->get_tower_at_channel(ihcal);
        ...
        if(!tInfo_ihc->get_isGood())
        {
            continue;
        }

        //if(tInfo_ihc->get_energy() < 0.2) continue;
        ...
        _ihcal_e.push_back(tInfo_ihc->get_energy());
        ...
        _ihcal_time.push_back(tInfo_ihc->get_time());
        _ihcal_chi2.push_back(tInfo_ihc->get_chi2());
        _ihcal_pedestal.push_back(tInfo_ihc->get_pedestal());
    }

    for(unsigned int ohcal = 0; ohcal < OHCAL_Container->size(); ohcal++)
    {
        tInfo_ohc = OHCAL_Container->get_tower_at_channel(ohcal);
        ...
        if(!tInfo_ohc->get_isGood())
        {
            continue;
        }

        //if(tInfo_ohc->get_energy() < 0.2) continue;
        ...
        _ohcal_e.push_back(tInfo_ohc->get_energy());
        ...
        _ohcal_time.push_back(tInfo_ohc->get_time());
        _ohcal_chi2.push_back(tInfo_ohc->get_chi2());
        _ohcal_pedestal.push_back(tInfo_ohc->get_pedestal());
    }
    ...
    // if(!hits_CEMC)
    // {
    //   std::cout << "EMiHCalo::process_event: G4HIT_CEMC not found!!!" << std::endl;
    // }
